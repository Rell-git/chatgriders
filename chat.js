...
function initUI(){
  ...
  if('serviceWorker' in navigator) navigator.serviceWorker.addEventListener('message',e=>{
    if(e.data?.type==='open-chat') switchPanel('chats');
  });
}
...
async function requestNotifPermission(){
  ...
  if(!('Notification' in window)){if(status)status.textContent='Notifications not supported on this browser.';return;}
  const p=await Notification.requestPermission();
  if(status) status.textContent = p==='granted'?'✅ Notifications enabled!':`${p==='denied'?'❌ Denied':'🕐 Not set'} — change in browser settings.`;
...
function updateDMBadge(){const b=document.getElementById('dm-badge');b.style.display=unreadCount>0?'':(b.style.display='none','none');b.textContent=unreadCount>9?'9+':unreadCount;}
...
function switchChatsTab(tab){
  ...
  document.getElementById('act-add').style.display=tab==='dm'?'flex':'none';
  document.getElementById('act-grp').style.display=tab==='groups'?'flex':'none';
...
function initPresence(){
  ...
  presenceCh.on('presence',{event:'sync'},()=>{const n=Object.keys(presenceCh.presenceState()).length;document.getElementById('online-label').textContent=n+' online';}).subscribe(async s=>{if(s==='SUBSCRIBED')await presenceCh.track({userId:ME.id});});
}

// ── Leaderboard ───────────────────────────────────────────────
async function pingLeaderboard(){
  const{data:ex}=await sb.from('leaderboard').select('score').eq('user_id',ME.id).maybeSingle();
  const score=(ex?.score||0)+60;
...
function addMsgCtx(el,msgId,table,isOwn){
  const show=(e)=>{ctxMsgId=msgId;ctxMsgTable=table;ctxMsgOwn=isOwn;const m=document.getElementById('ctx-menu');m.style.display='block';m.style.left=Math.min(e.clientX||e.pageX,window.innerWidth-160)+'px';m.style.top=Math.min(e.clientY||e.pageY,window.innerHeight-80)+'px';document.getElementById('ctx-delete-btn').firstChild.nextSibling.textContent=isOwn?' Delete for everyone':' Delete for me';};
...
function subscribeIncomingDMs(){
  incomingDMSub=sb.channel(`incoming-${ME.id}`).on('postgres_changes',{event:'INSERT',schema:'public',table:'private_messages',filter:`receiver_id=eq.${ME.id}`},async pl=>{
    ...
    if(!ex){const{data:p}=await sb.from('profiles').select('*').eq('id',sid).single();if(p)prependConvoItem(p,pl.new.content||'📷',false);}
    else{ex.querySelector('.convo-preview').textContent=pl.new.content||'📷';const list=document.getElementById('convo-list');if(list.firstChild!==ex)list.insertBefore(ex,list.firstChild);}
...
async function openThread(userId,name,seed,style,avatarUrl){
  ...
  scrollBottom(area);
  const last=[...(data||[])].reverse().find(m=>m.sender_id===ME.id&&m.seen_at);
  showSeenBar(last?.seen_at);
...
function applyCtTheme(theme,save=true){
  ...
  // Store on element for bubble color
  if(area) area.dataset.ct=theme;
  // Update bubble colors via CSS variable
...
function appendPrivateMsg(msg){
  ...
  const isLoc=msg.content?.startsWith('📍 Location:');
  ...
  if(isPoll)       inner=renderPollCard(msg.content);
    ...
    if(msg.content)   bubble+=`<div class="msg-text">${linkify(msg.content)}</div>`;
    if(msg.image_url) bubble+=`<img class="msg-img" src="${escHtml(msg.image_url)}" loading="lazy" onclick="openImgFull(this.src)">`;
    inner=`<div class="msg-bubble" style="${isMine?`background:var(--ct-bubble);border-color:var(--ct-bubble);color:#fff`:''}">${bubble}</div>`;
  }

  const el=document.createElement('div');
  el.className=`msg${isMine?' mine':''}${(isPoll||isTrade||isLoc)?' msg-card-wrap':''}`;
...
function renderPollCard(content){
  const lines=content.split('\n'),q=lines[0].replace('📊 Poll: ','');
  const opts=lines.slice(1).filter(Boolean);
  const optHtml=opts.map((l,i)=>`
    <div class="poll-option" onclick="this.querySelector('.poll-fill').style.width='100%'">
      <div class="poll-option-bar"><div class="poll-fill"></div></div>
      <span class="poll-option-label">${escHtml(l)}</span>
    </div>`).join('');
  return `<div class="poll-card-standalone"><div class="poll-card-hdr">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="14" height="14"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/><line x1="2" y1="20" x2="22" y2="20"/></svg>
    Poll</div><div class="poll-card-q">${escHtml(q)}</div><div class="poll-options">${optHtml}</div></div>`;
}
function renderTradeCard(content,imageUrl){
  const lines=content.split('\n');
  ...
  return `<div class="trade-card-standalone">
    ${imageUrl?`<img class="trade-card-img" src="${escHtml(imageUrl)}" onclick="openImgFull(this.src)" loading="lazy">`:''}
    <div class="trade-card-hdr"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="14" height="14"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg> Trade Offer</div>
    <div class="trade-card-cols">
      <div class="trade-card-col"><div class="trade-card-lbl">I Offer</div><div class="trade-card-val">${escHtml(offer)}</div></div>
      <div class="trade-card-div">⇄</div>
      <div class="trade-card-col"><div class="trade-card-lbl">I Want</div><div class="trade-card-val">${escHtml(want)}</div></div>
    </div>
    ${note?`<div class="trade-card-note">📝 ${escHtml(note)}</div>`:''}
  </div>`;
}
function renderLocationCard(content){
  const url=content.replace('📍 Location: ','');
  const coords=url.split('?q=')[1]||'';
  const [lat,lng]=(coords.split(','));
  const mapImg=lat&&lng?`https://staticmap.openstreetmap.de/staticmap.php?center=${lat},${lng}&zoom=14&size=300x140&markers=${lat},${lng}&format=png`:'';
  return `<div class="loc-card-standalone">
    <div class="loc-card-hdr"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="14" height="14"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg> Location</div>
    ${mapImg?`<img class="loc-map-img" src="${escHtml(mapImg)}" loading="lazy" alt="Map">`:''}
    <div class="loc-card-foot"><a href="${escHtml(url)}" target="_blank" rel="noopener" class="btn-accent-sm">Open in Maps</a></div>
  </div>`;
}
...
async function sendTrade(){
  ...
  let imageUrl=null;
  if(tradeImageFile){const path=`trade/${ME.id}/${Date.now()}-${tradeImageFile.name}`;await sb.storage.from('chat-images').upload(path,tradeImageFile);const{data:{publicUrl}}=sb.storage.from('chat-images').getPublicUrl(path);imageUrl=publicUrl;}
  document.getElementById('trade-modal').style.display='none';
  await sb.from('private_messages').insert({sender_id:ME.id,receiver_id:activeUser,content:`🤝 Trade Offer\nOffer: ${offer}\nWant: ${want}${note?'\nNote: '+note:''}`,image_url:imageUrl||undefined});
}
...
function switchAddTab(tab){
  document.querySelectorAll('#add-modal .ctab').forEach(b=>b.classList.toggle('active',b.textContent.toLowerCase().includes(tab==='code'?'code':'scan')));
  document.getElementById('add-tab-code').style.display=tab==='code'?'':'none';
  document.getElementById('add-tab-scan').style.display=tab==='scan'?'':'none';
  if(tab==='scan') startQRScanner(); else stopQRScanner();
}
function startQRScanner(){
  if(html5QrScanner) return;
  html5QrScanner=new Html5Qrcode('qr-reader');
  html5QrScanner.start({facingMode:'environment'},{fps:10,qrbox:200},(code)=>{
    stopQRScanner();
    // Extract user code from QR (format: pulseship://u/000001 or just the code)
    const match=code.match(/(\d{6})/);
    if(match){document.getElementById('add-code-input').value=match[1];switchAddTab('code');searchByCode();}
    else{document.getElementById('qr-scan-result').textContent='Invalid QR code.';}
  }).catch(()=>{document.getElementById('qr-scan-result').textContent='Camera access denied.';});
}
...
async function openProfileModal(userId){
  if(!userId) return;
  if(userId===ME.id){openSettings();return;}
...
function openSPage(id){
  // Slide current page left, slide new page in from right
  ...
  next.style.transform='translateX(100%)';next.classList.add('active');
...
function closeSPage(){
  const cur=document.querySelector('.spage.active');
  if(!cur||cur.id==='sp-main') return;
  cur.style.transform='translateX(100%)';
  setTimeout(()=>{cur.classList.remove('active');cur.style.transform='';},280);
  sPageStack.pop();
  const prevId=sPageStack[sPageStack.length-1]||'sp-main';
...
function loadSettingsProfile(){
  const av=getAvatar(ME.avatar_seed,ME.avatar_style,ME.avatar_url);
  const spcAv=document.getElementById('spc-avatar');
  if(spcAv) spcAv.src=av;
  document.getElementById('nav-avatar').src=av;
  ...
  const ring=document.getElementById('spc-border-ring');
  if(ring)ring.className=`avatar-border-ring border-ring-${ME.border_style||'none'}`;
}
async function quickPhotoUpload(input){
  ...
  ME.avatar_url=publicUrl;loadSettingsProfile();
}
...
async function saveProfile(){
  ...
  Object.assign(ME,{...upd,name:name||ME.name});loadSettingsProfile();closeSPage();
}
...
function renderShareProfile(){
  ...
  container.innerHTML='';
  const code=ME.user_code||'000000';
  const qrData=`pulseship://u/${code}`;
  try{new QRCode(container,{text:qrData,width:160,height:160,colorDark:document.documentElement.getAttribute('data-theme')==='light'?'#111':'#7b6ef6',colorLight:'transparent',correctLevel:QRCode.CorrectLevel.H});}catch(e){container.textContent='QR unavailable';}
...
async function submitReport(){
  const type=document.getElementById('report-type').value,desc=document.getElementById('report-desc').value.trim();
  if(!desc)return alert('Please describe the issue.');
  await sb.from('private_messages').insert({sender_id:ME.id,receiver_id:ME.id,content:`[REPORT] Type: ${type}\n${desc}`}).catch(()=>{});
  alert('Report sent. Thank you!');closeSPage();
}
...
async function deleteAccount(){
  ...
  if(typed.toLowerCase()!==displayName(ME).toLowerCase()){err.textContent=`Type exactly: "${displayName(ME)}"`;err.style.display='';return;}
...
async function loadGroups(){
  ...
  list.innerHTML=(groups||[]).map(g=>`<div class="convo-item" data-gid="${g.id}" onclick="openGroup(${g.id},'${escAttr(g.name)}')"><div class="convo-av" style="background:var(--sur2);display:flex;align-items:center;justify-content:center">
    <svg viewBox="0 0 24 24" fill="none" stroke="var(--muted)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="20" height="20"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
...
// Drag
(function(){let dx=0,dy=0,drag=false,el,sx,sy;document.addEventListener('DOMContentLoaded',()=>{el=document.getElementById('toolbox');const hdr=document.getElementById('toolbox-drag-hdr');if(!hdr)return;hdr.addEventListener('mousedown',e=>{if(e.target.closest('.tbtn,.tbtn-ctrl'))return;drag=true;const r=el.getBoundingClientRect();dx=e.clientX-r.left;dy=e.clientY-r.top;e.preventDefault();});document.addEventListener('mousemove',e=>{if(!drag)return;el.style.left=(e.clientX-dx)+'px';el.style.top=(e.clientY-dy)+'px';el.style.right='auto';el.style.bottom='auto';});document.addEventListener('mouseup',()=>drag=false);});})();
...
function calcAction(v){
  ...
  if(v==='clear'){calcValue='0';calcPrev='';calcOp='';calcReset=false;}
    ...
    const a=parseFloat(calcPrev),b=parseFloat(calcValue);
    let r=0;if(calcOp==='+')r=a+b;else if(calcOp==='−')r=a-b;else if(calcOp==='×')r=a*b;else if(calcOp==='÷')r=b!==0?a/b:0;
    const hist=document.getElementById('calc-history');hist.textContent=`${calcPrev} ${calcOp} ${calcValue} =`;
    calcValue=r.toString();calcOp='';calcPrev='';calcReset=false;
  }
  else if(v==='.'){if(calcReset){calcValue='0';calcReset=false;}if(!calcValue.includes('.'))calcValue+='.'}
  else{if(calcReset||calcValue==='0'){calcValue=v;calcReset=false;}else calcValue+=v;}
  disp.textContent=parseFloat(calcValue).toLocaleString('en',{maximumFractionDigits:10})||'0';
}

// Translator
async function translateNow(){
  const txt=document.getElementById('tl-input').value.trim();const out=document.getElementById('tl-output');
  if(!txt){out.textContent='Translation will appear here';return;}
  const from=document.getElementById('tl-from').value,to=document.getElementById('tl-to').value;
  out.textContent='Translating…';
  try{const res=await fetch(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(txt)}&langpair=${from}|${to}`);const d=await res.json();out.textContent=d.responseData?.translatedText||'Translation failed.';}catch{out.textContent='Network error.';}
}
...
function copyTranslation(){const txt=document.getElementById('tl-output').textContent;navigator.clipboard?.writeText(txt).then(()=>alert('Copied!'));}
...
const DRAW_PROMPTS = {
  ...
  'Places':        ['Beach','Mountain','Castle','Lighthouse','Library','Space station','Treehouse','Volcano','Waterfall','City skyline','Farm','Desert oasis','Pirate ship','Moon base','Jungle'],
  'Vehicles':      ['Race car','Spaceship','Submarine','Hot air balloon','Skateboard','Helicopter','Train','Sailboat','Motorbike','Tank','Hovercraft','Blimp','Kayak','Jeep','Fire truck'],
  'Actions':       ['Surfing','Sleeping','Dancing','Flying','Reading','Fighting','Swimming','Cooking','Laughing','Running','Jumping','Fishing','Singing','Painting','Meditating'],
};
const PROMPT_CATEGORIES = Object.keys(DRAW_PROMPTS);

let drawCanvas=null, drawCtx=null, drawSub=null, drawIsDrawing=false;
let drawColor='#7b6ef6', drawSize=5, drawEraser=false, drawStrokes=[], gameIsFs=false;
...
function newDrawPrompt(){
  const cat=PROMPT_CATEGORIES[Math.floor(Math.random()*PROMPT_CATEGORIES.length)];
  const words=DRAW_PROMPTS[cat];
  const word=words[Math.floor(Math.random()*words.length)];
  const el=document.getElementById('draw-prompt-word');
  if(el) el.textContent=`${word} (${cat})`;
}
...
function toggleGameFs(){
  gameIsFs=!gameIsFs;
  document.getElementById('game-modal').classList.toggle('game-modal-fs',gameIsFs);
  setTimeout(()=>{resizeCanvas();redrawAll();},60);
}

function resizeCanvas(){
  if(!drawCanvas)return;
  const inner=document.getElementById('game-modal-inner');
  drawCanvas.width=inner.clientWidth;
  // height = total inner height minus header+prompt+tools+footer rows
  const headers=inner.querySelectorAll('.game-modal-hdr,.draw-prompt-banner,.game-tools,.game-footer-bar');
  let taken=0;headers.forEach(el=>taken+=el.offsetHeight);
  drawCanvas.height=Math.max(200,inner.clientHeight-taken);
}
...
function initDrawGame(){
  ...
  // Color dots
  const dotsEl=document.getElementById('draw-color-dots');
  ...
  // Brush size input
  document.getElementById('brush-size').oninput=e=>drawSize=+e.target.value;

  resizeCanvas();
  new ResizeObserver(()=>{resizeCanvas();redrawAll();}).observe(document.getElementById('game-modal-inner'));

  // Mouse events
  drawCanvas.addEventListener('mousedown',e=>{startDraw(getPos(e));});
  drawCanvas.addEventListener('mousemove',e=>{if(drawIsDrawing)continueDraw(getPos(e));});
  drawCanvas.addEventListener('mouseup',endDraw);
  drawCanvas.addEventListener('mouseleave',endDraw);
  // Touch events
  drawCanvas.addEventListener('touchstart',e=>{e.preventDefault();startDraw(getPos(e.touches[0]));},{passive:false});
  drawCanvas.addEventListener('touchmove', e=>{e.preventDefault();if(drawIsDrawing)continueDraw(getPos(e.touches[0]));},{passive:false});
...
function startDraw(p){
  drawIsDrawing=true;currentStrokePts=[p];
  drawCtx.beginPath();drawCtx.moveTo(p.x,p.y);
}
function continueDraw(p){
  if(!drawIsDrawing)return;
  currentStrokePts.push(p);
  drawCtx.lineWidth=drawEraser?24:drawSize;
  drawCtx.strokeStyle=drawEraser?'#ffffff':drawColor;
  drawCtx.lineCap='round';drawCtx.lineJoin='round';
  drawCtx.lineTo(p.x,p.y);drawCtx.stroke();
  drawCtx.beginPath();drawCtx.moveTo(p.x,p.y);
}
async function endDraw(){
  if(!drawIsDrawing||currentStrokePts.length<2){drawIsDrawing=false;currentStrokePts=[];return;}
  drawIsDrawing=false;
  const stroke={color:drawEraser?'#ffffff':drawColor,size:drawEraser?24:drawSize,pts:currentStrokePts};
  currentStrokePts=[];
  drawStrokes.push(stroke);
  // Save to Supabase
  if(ME) await sb.from('draw_strokes').insert({room_id:'world',user_id:ME.id,data:stroke}).catch(()=>{});
}

function drawStroke(s){
  if(!s.pts||s.pts.length<2)return;
  drawCtx.beginPath();drawCtx.moveTo(s.pts[0].x,s.pts[0].y);
  s.pts.forEach(p=>drawCtx.lineTo(p.x,p.y));
  drawCtx.lineWidth=s.size;drawCtx.strokeStyle=s.color;
...
async function loadDrawStrokes(){
  const{data}=await sb.from('draw_strokes').select('data,user_id').eq('room_id','world').order('created_at',{ascending:true}).limit(300).catch(()=>({data:[]}));
  drawStrokes=(data||[]).map(r=>r.data).filter(Boolean);
  redrawAll();
}
function subscribeDrawStrokes(){
  if(drawSub)sb.removeChannel(drawSub);
  drawSub=sb.channel('draw-world').on('postgres_changes',{event:'INSERT',schema:'public',table:'draw_strokes',filter:'room_id=eq.world'},pl=>{
    if(pl.new.user_id!==ME?.id){const s=pl.new.data;if(s){drawStrokes.push(s);drawStroke(s);}}
  }).on('postgres_changes',{event:'DELETE',schema:'public',table:'draw_strokes',filter:'room_id=eq.world'},()=>{drawStrokes=[];redrawAll();}).subscribe();
}
async function clearMyStrokes(){
  if(!ME)return;
  await sb.from('draw_strokes').delete().eq('user_id',ME.id).eq('room_id','world').catch(()=>{});
  await loadDrawStrokes();
}
function setDrawColor(c){
  drawColor=c;drawEraser=false;
  document.querySelectorAll('.draw-color-dot').forEach(d=>d.classList.toggle('active',d.style.background===c));
  document.getElementById('eraser-btn')?.classList.remove('active');
}
function toggleEraser(){
  drawEraser=!drawEraser;
  document.getElementById('eraser-btn')?.classList.toggle('active',drawEraser);
}
function initGameDrag(){
  ...
  hdr.addEventListener('mousedown',e=>{
    ...
    drag=true;const r=inner.getBoundingClientRect();dx=e.clientX-r.left;dy=e.clientY-r.top;e.preventDefault();
  });
  document.addEventListener('mousemove',e=>{if(!drag||gameIsFs)return;inner.style.left=(e.clientX-dx)+'px';inner.style.top=(e.clientY-dy)+'px';inner.style.right='auto';inner.style.bottom='auto';});
